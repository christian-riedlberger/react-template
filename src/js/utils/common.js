...
export function replaceChildren(obj: Array<Object>, replace: any) {
...
export function updateChildren(obj: Object, children: Object) {
    return _.transform(obj, (result, value, key) => {
        if (
            ...
            result[key] = key === 'name' ? children.name : children.description;
        } else {
            result[key] =
                _.isArray(value) || _.isObject(value)
        ...
        if (key === 'nodeRef' && value === children.parentId) {
            result.children = _.sortBy(result.children, 'name');
        }
    });
}
...
export function mergeTableColumns(
    defaultColumns: Array<Object>,
    prefColumns?: Array<Object>
) {
    const tableColumns = _.map(defaultColumns, column => {
        const prefColumn = _.find(prefColumns, { name: column.name });

        if (prefColumn && _.isNil(prefColumn.options))
            return {
                ...column,
                options: {
                    ...column.options,
                    display: prefColumn.display
                }
            };

        const options =
            prefColumn && !_.isNil(prefColumn.options.display)
                ? {
                      ...column.options,
                      display: prefColumn.options.display
                  }
                : {
                      ...column.options,
                      display: true
